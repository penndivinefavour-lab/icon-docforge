...
(function () {
  ...
  function showResult(file) {
    ...
    window.App.currentOutputUri = file.uri;
    window.App.currentOutputMime = file.mime || 'application/pdf';
    navigate('result');
  ...
  document.getElementById('btn-open')?.addEventListener('click', () => {
    if (window.AndroidBridge && window.App.currentOutputUri) {
      AndroidBridge.openFile(window.App.currentOutputUri, window.App.currentOutputMime || 'application/pdf');
    } else {
      showToast('No output file available', 'info');
    }
  });

  document.getElementById('btn-share')?.addEventListener('click', () => {
    if (window.AndroidBridge && window.App.currentOutputUri) {
      AndroidBridge.shareFile(window.App.currentOutputUri, window.App.currentOutputMime || 'application/pdf');
